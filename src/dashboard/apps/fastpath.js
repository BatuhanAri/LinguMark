...
function renderUnits(units, currentStepIndex, historyForLang) {
    ...
    units.forEach((unit, unitIdx) => {
        ...
        unitSteps.forEach((chunk, stepOffset) => {
            ...
            const isCompleted = index < currentStepIndex || historyForLang[index] !== undefined;
            const isFirstStepOfUnit = index === unitStartStep;
            const isPrevStepCompleted = index > unitStartStep && (index - 1 < currentStepIndex || historyForLang[index - 1] !== undefined);
            
            const isLocked = !isFirstStepOfUnit && !isPrevStepCompleted && !isCompleted;
            ...
            if (isActive) {
                ...
                // Unlocked but not yet active future step
