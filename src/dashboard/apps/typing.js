...
export function initTyping(lang = 'tr') {
  ...
  function showQuestion() {
      ...
      container.innerHTML = `
        ...
        <div class="bg-white/[0.03] backdrop-blur-3xl border border-white/10 p-12 rounded-[48px] text-center shadow-3xl relative overflow-hidden w-full group">
            ...
            <p class="text-slate-500 mb-6 tracking-[0.2em] uppercase text-xs font-black relative z-10">${t('yazim_talimati', lang)}</p>
            <h2 class="text-3xl sm:text-5xl font-black text-white mb-12 tracking-tighter drop-shadow-2xl relative z-10 break-words font-['Outfit']">${currentWord.meaning}</h2>
            
            <div class="relative z-10 max-md mx-auto">
                ...
                <div id="feedback" class="text-sm font-black h-8 mb-6 mt-[-15px] hidden uppercase tracking-widest text-[11px]"></div>
                <div class="flex gap-4">
                    <button id="btnHint" class="bg-white/5 hover:bg-white/10 text-slate-400 font-black py-4 px-8 rounded-[24px] transition-all border border-white/5 w-1/3 text-sm uppercase tracking-wider active:scale-95 text-xs text-[10px]">${t('ipucu', lang)}</button>
                    <button id="btnSubmit" class="bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white font-black py-4 px-8 rounded-[24px] shadow-2xl transition-all border border-white/10 w-2/3 uppercase tracking-widest active:scale-95 text-[12px]">${t('kontrol_et', lang)}</button>
                </div>
            </div>
        </div>
      `;
      ...
      const checkAnswer = () => {
          ...
          if(val === target) {
              ...
              feedback.textContent = t('dogru', lang);
              ...
              feedback.textContent = `${t('yanlis', lang)} ${currentWord.word}`;
